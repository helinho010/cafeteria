...
if (!empty($_POST["mes"]) && !empty($_POST["anio"])) 
{
    ...
    $query = mysqli_query($conexion, 'select usuarios.id, usuarios.nombre,	tmp.total from	usuarios join ( select 	pedidos.id_usuario , SUM(pedidos.total) as total from pedidos join usuarios on usuarios.id = pedidos.id_usuario where usuarios.estado = 1 and MONTH (pedidos.fecha) = '.$_POST["mes"].' and YEAR (pedidos.fecha) = '.$_POST["anio"].' GROUP BY	pedidos.id_usuario ) tmp on tmp.id_usuario = usuarios.id where usuarios.estado = 1'); 
    ...
    $query = mysqli_query($conexion, 'select usuarios.id, usuarios.nombre,	tmp.total from	usuarios join ( select 	pedidos.id_usuario , SUM(pedidos.total) as total from pedidos join usuarios on usuarios.id = pedidos.id_usuario where usuarios.estado = 1 and MONTH (pedidos.fecha) = '.date('m').' and YEAR (pedidos.fecha) = '.date('Y').' GROUP BY	pedidos.id_usuario ) tmp on tmp.id_usuario = usuarios.id where usuarios.estado = 1'); 
